...
**请认真阅读实验文档**，助教不负责回答因为不认真阅读文档而产生的问题。

然而，如果同学们认为文档和测评程序有任何不合理/不清晰的地方，欢迎带着你们的思考和建议联系助教刘俊宇、曾启桓。
...
为了优化RTP在这种情况下的性能，你需要相应地修改你的`receiver`和`sender`，将其重传逻辑修改为**选择重传**。具体来说:
...
> - 尽管如此，由于滑动窗口的规模需要维护，`receiver`仍然需要维护一个`N`，表示期望收到的下一个`DATA`报文的`seq_num`。对于携带任意大于或等于`N+window_size`的`seq_num`的`DATA`报文应当直接丢弃。而`sender`同样需要保留滑动窗口的控制逻辑来进行流量控制。由于`ACK`报文的含义发生变化，滑动窗口移动的方式也会有所变化。
