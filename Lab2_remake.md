...
lab2的文档中包含一些引导思考（标有[?]）和提供建议（标有[!]）的文本，它们提及的问题和任务并不会计入lab2的分数中。尽管如此，它们还是会为你完成lab提供或多或少的帮助，请不要遗漏这些内容。

如果对文档有任何的建议，欢迎联系助教曾启桓。
...
其中，MSG_DONTWAIT表示不阻塞，立即返回执行结果。由于UDP并不是TCP那种面向连接的传输层协议，每一次收发都需要指明对侧的地址。
...
对于每一个被确认接收的`START`报文或`END`报文，你需要发送一个`ACK`报文，该报文的`seq_num`已在`sender`的实现要求中描述。
