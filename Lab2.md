...
本lab的预计**最大耗时**在20个小时左右。如果你在完成lab上花费了更多的时间，并且仍然面对难以解决的问题，请带着你的困扰尽快求助教学团队。
...
你需要分别实现一个RTP协议的Sender（发送端）和Receiver（接收端）。其中，Sender使用RTP协议将一个文件发送给Receiver，而Receiver则使用RTP协议接收文件，并将文件的内容写入指定的路径。

RTP的工作流程共分为三个阶段，分别是**建立连接、传输数据和终止连接**。在传输数据阶段，Sender和Receiver需要分别实现回退N和选择重传算法。此外，网络错误可能在任意阶段出现，你需要在实现中充分应对可能出现的差错。
...
* `flags`：报文类型标志。它的高5位暂时未被使用（需要置0），低3位可以标识报文类型，具体使用方法请参考文档在建立连接部分的说明。
...
成功连接之后，Sender发送的第一个数据报文的`seq_num`应该为$x+1$（假设SYN报文的编号为$x$）。此后，每个新发送的数据报文（不包括重传）需要将编号递增1。Sender需要将文件数据进行适当的分片，并且根据滑动窗口算法逐渐将这些数据发送给Receiver，并且在适当的时机重传丢失或损坏的数据。
